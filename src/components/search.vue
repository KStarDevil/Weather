...
export default {
    ...
    methods: {
        ...
        search(loading, search) {
            axios("https://places-dsn.algolia.net/1/places/query", {
                ...
                response.data.hits.forEach(hit => {
                    if(hit.administrative != undefined){
        ...
        async weatherLocation() {
            ...
            await axios(`https://api.openweathermap.org/data/2.5/forecast/daily?q=${city}&units=metric&appid=${this.$apiKey}`).then((res) => {
                this.$emit('seven', res.data)

            })
            await axios(`https://api.openweathermap.org/data/2.5/weather?q=${city}&units=metric&appid=${this.$apiKey}`).then((res) => {
        ...
        async getGeolocation(data) {
            await axios(`https://api.openweathermap.org/data/2.5/forecast/daily?lat=${data.coords.latitude}&lon=${data.coords.longitude}&units=metric&appid=${this.$apiKey}`).then((res) => {
                this.$emit('seven', res.data)
            });

            await axios(`https://api.openweathermap.org/data/2.5/weather?lat=${data.coords.latitude}&lon=${data.coords.longitude}&units=metric&appid=${this.$apiKey}`)
                .then((res) => {
                    this.$emit('daily', res.data)
                })
